...
void info_command::route(const dpp::slashcommand_t &event) {
	...
	db::resultset counts = db::query("SELECT COUNT(*) as guild_total, SUM(user_count) as user_total FROM guild_cache");
	...
	dpp::embed embed = dpp::embed()
		...
		.add_field("Total Servers", guild_count, true)
		.add_field("Total Users", user_count, true)
	...
	embed.add_field("Support Server", "[Beholder Support](https://discord.com/invite/Ac2duhZAr5)", false);
